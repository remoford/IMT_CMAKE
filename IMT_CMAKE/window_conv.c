...
struct my_f_params { double m; double s; };
...
double my_f(double x, void * p) {
	...
	distType probabilityDensity[4];

	waldpdf(&time, m, s, &probabilityDensity, 4);

	return probabilityDensity[0];
}

distType get_window_bisection(double m, double s, long * firstIdx, long * lastIdx, distType threshold) {
	...
	struct my_f_params params = { m, s };

	F.function = &my_f;

	F.params = &params;

	gsl_root_fsolver_set(S, &F, DBL_MIN, mean);
...
void window_conv(const distType z[], const distType y[], distType C[], double h, unsigned long size_xyz)
{
	...
	long firstIdx, lastIdx, firstYIdx, lastYIdx;

	get_window(z, y, &firstIdx, &lastIdx, &firstYIdx, &lastYIdx, threshold, size_xyz);
